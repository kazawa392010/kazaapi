...
async def submit_form(req: SubmitFormRequest):
    ...
    # BUGFIX: previously always posted back to req.url (the page itself)
    # since Jina's markdown strips the real <form action=...>. Resolve the
    # actual submit target with a direct (non-Jina) fetch first.
    action_url = await agent.resolve_form_action(req.url)
    try:
        result = await agent.submit_form(req.url, action_url, req.known_values)
    except Exception as exc:
...
async def search_images(query: str = Query(...), max_results: int = Query(6, ge=1, le=20)):
    ...
    JSON API, so we reuse the free web-search backend's regular web results
