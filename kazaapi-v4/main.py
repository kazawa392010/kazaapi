...
async def _self_learning_task() -> None:
    """Scrapes a random topic (via free web search -> first result -> Jina
    Reader, with a resilient fallback per services/search.py and
    services/agent.py's BUGFIXES) and folds a summary into a shared
    'uri-self-learning' long-term memory bucket, so Uri can casually bring
    it up later."""
    import database
    ...
    try:
        # BUGFIX: use whatever provider keys the owner configured via
        # /api/v1/config, same as routers/chat.py — previously this call
        # only ever worked if a key happened to also be set as a Render
        # env var, even though the LLM key might only be saved in Supabase.
